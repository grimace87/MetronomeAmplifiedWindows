...
Concurrency::task<void> vbo::SettingsHubLabelsVertexBuffer::MakeInitTask(DX::DeviceResources* resources)
{
	// Coordinates used in the vertex buffer depend on the window size
	return Concurrency::create_task([this, resources]() -> void {
		...
		int bufferIndex = 0;
		const float maxTextHeightPixels = 1.2f * marginLogicalInches * dpi;
        orkney->PrintTextIntoVboCentredInside(vboData, bufferIndex, labels[0], leftX, t1, rightX - leftX, t1 - t2, maxTextHeightPixels, size);
        bufferIndex += 6 * labels[0].length();
        orkney->PrintTextIntoVboCentredInside(vboData, bufferIndex, labels[1], leftX, t2, rightX - leftX, t2 - t3, maxTextHeightPixels, size);
        bufferIndex += 6 * labels[1].length();
        orkney->PrintTextIntoVboCentredInside(vboData, bufferIndex, labels[2], leftX, t3, rightX - leftX, t3 - t4, maxTextHeightPixels, size);
        bufferIndex += 6 * labels[2].length();
        orkney->PrintTextIntoVboCentredInside(vboData, bufferIndex, labels[3], leftX, t4, rightX - leftX, t4 - t5, maxTextHeightPixels, size);
        bufferIndex += 6 * labels[3].length();
        orkney->PrintTextIntoVboCentredInside(vboData, bufferIndex, labels[4], leftX, t5, rightX - leftX, t5 - t6, maxTextHeightPixels, size);
        bufferIndex += 6 * labels[4].length();
