...
void MainSceneRenderer::Render()
{
	...
	if (!m_deviceDependentLoadingComplete || !m_windowDependentLoadingComplete || !m_deviceResources->AreShadersFulfilled())
	{
		return;
	}
...
void MainSceneRenderer::CreateDeviceDependentResources()
{
	...
	(createTextureTask).then([this](Concurrency::task<void> t) {
		try {
			t.get();
		}
...
void MainSceneRenderer::CreateWindowSizeDependentResources()
{
	...
	auto createVertexBufferTask = create_task([this] {
		...
		VertexTexCoord sceneVertices[114];
